...
through the `restore` callback on the next visit or when the browser's back
button lands on a different address. Name the `element` too, so that the
filtering is released if the Client Extension is taken off the page without
disconnecting.
...
const connection = new FDSConnection(
	fdsName,
	{
		restore: (connectionState) => {
			...
			const selection = parseSelection(connectionState);
	...
	{appId: 'mySampleFilterApp', element, owns: ['filters']}
);
```
